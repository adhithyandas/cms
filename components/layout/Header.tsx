...
export function Header() {
  ...
  return (
    <header
      ref={headerRef}
      className="fixed top-0 z-50 w-full flex items-center h-20"
    >
      <div
        ref={containerRef}
        className="container mx-auto px-4 lg:px-8 h-full flex items-center"
      >
        {/* Logo */}
        <Link href="/" className="flex items-center space-x-3 shrink-0">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm leading-tight text-gray-400 mt-0.5">
              Academy
            </span>
          </div>
        </Link>
          ...
          <nav className="flex flex-col p-4 space-y-4 text-base">
            {navLinks.map((link) => (
              <Link
                ...
                className={
                  link.active
                    ? "text-red-500 py-2 border-b border-white/5 font-medium"
                    : "text-gray-300 hover:text-red-500 transition-colors py-2 border-b border-white/5"
                }
